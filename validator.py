...
1. BUILD: Build reference corpus from real CloudTrail logs
   python cloudtrail_validator.py --build /path/to/real/logs/

2. VALIDATE: Validate test logs against corpus
   python cloudtrail_validator.py test.json [--csv]
"""
...
def load_json_file(filepath: Path) -> List[Dict]:
    """Load JSON or gzipped JSON file, return list of events."""
    try:
...
def build_corpus(logs_dir: Path) -> Dict[str, Any]:
    """
    Build corpus database from real CloudTrail logs.
    ...
    for filepath in all_files:
        ...
        if file_count % 100 == 0:
            print(f"  Processed {file_count} files, {event_count} events, {len(all_field_paths)} unique field paths...")
    
    print(f"\n✅ Corpus build complete:")
    print(f"   Files processed: {file_count}")
    print(f"   Events processed: {event_count}")
    print(f"   Unique field paths: {len(all_field_paths)}")
